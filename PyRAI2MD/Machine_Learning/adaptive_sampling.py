...
class AdaptiveSampling:
    """ Adaptive sampling class
        ...
        Attribute:           Type:
            ...
            dynsample        int         sample geometries using dynamical error
            ...
            fwdenergy        int         number of iteration to delay raising dynamical energy error
            bckenergy        int         number of iteration to delay reducing dynamical energy error
            ...
            fwdgrad          int         number of iteration to delay raising dynamical gradient error
            bckgrad          int         number of iteration to delay reducing dynamical gradient error
            ...
            fwdnac           int         number of iteration to delay raising dynamical nac error
            bcknac           int         number of iteration to delay reducing dynamical nac error
            ...
            fwdsoc           int         number of iteration to delay raising dynamical soc error
            bcksoc           int         number of iteration to delay reducing dynamical soc error
            itr              int         number of search iteration
            ...
            nac              list        recorded non-adiabatic couplings per trajectory
            ...
            err_n            list        recorded non-adiabatic coupling errors per trajectory
    ...
    def __init__(self, keywords=None):
        ...
        if self.dynsample == 0:
            self.dyn_e = [self.maxenergy for _ in range(ninitcond)]
            self.dyn_g = [self.maxgrad for _ in range(ninitcond)]
            self.dyn_n = [self.maxnac for _ in range(ninitcond)]
            self.dyn_s = [self.maxsoc for _ in range(ninitcond)]
        else:
            self.dyn_e = [self.minenergy + self.inienergy * (self.maxenergy - self.minenergy) for _ in range(ninitcond)]
            self.dyn_g = [self.mingrad + self.inigrad * (self.maxgrad - self.mingrad) for _ in range(ninitcond)]
            self.dyn_n = [self.minnac + self.ininac * (self.maxnac - self.minnac) for _ in range(ninitcond)]
            self.dyn_s = [self.minsoc + self.inisoc * (self.maxsoc - self.minsoc) for _ in range(ninitcond)]
        ...
        self.itr_e = [0 for _ in range(ninitcond)]
        self.itr_g = [0 for _ in range(ninitcond)]
        self.itr_n = [0 for _ in range(ninitcond)]
        self.itr_s = [0 for _ in range(ninitcond)]
        ...
        initcond = sampling(self.title, ninitcond, gl_seed, temp, method, ld_format)
    ...
    def _abinit_wrapper(self, variables):
        ...
        ## run QC calculation
        qc = QM(self.abinit, keywords=self.keywords, job_id=geom_id + 1)
        mol = qc.evaluate(mol)
    ...
    def _screen_error(self, md_traj):
        ...
        ntraj = len(md_traj)
        md_last = [[] for _ in range(ntraj)]
        md_final = [[] for _ in range(ntraj)]
        md_atoms = [[] for _ in range(ntraj)]
        md_geom = [[] for _ in range(ntraj)]
        md_energy = [[] for _ in range(ntraj)]
        md_grad = [[] for _ in range(ntraj)]
        md_nac = [[] for _ in range(ntraj)]
        md_soc = [[] for _ in range(ntraj)]
        md_err_e = [[] for _ in range(ntraj)]
        md_err_g = [[] for _ in range(ntraj)]
        md_err_n = [[] for _ in range(ntraj)]
        md_err_s = [[] for _ in range(ntraj)]
        md_dyn_e = [[] for _ in range(ntraj)]
        md_dyn_g = [[] for _ in range(ntraj)]
        md_dyn_n = [[] for _ in range(ntraj)]
        md_dyn_s = [[] for _ in range(ntraj)]
        md_itr_e = [[] for _ in range(ntraj)]
        md_itr_g = [[] for _ in range(ntraj)]
        md_itr_n = [[] for _ in range(ntraj)]
        ...
        md_ndiscard = [[] for _ in range(ntraj)]
        md_nrefine = [[] for _ in range(ntraj)]
        md_selec_e = [[] for _ in range(ntraj)]
        ...
        for val in pool.imap_unordered(self._screen_error_wrapper, variables_wrapper):
            ...
            md_energy[traj_id] = traj_data[5].tolist()  # all energies
            md_grad[traj_id] = traj_data[6].tolist()  # all forces
            md_nac[traj_id] = traj_data[7].tolist()  # all NACs
            md_soc[traj_id] = traj_data[8].tolist()  # all SOCs
            md_err_e[traj_id] = traj_data[9].tolist()  # all prediction error in energies
            md_err_g[traj_id] = traj_data[10].tolist()  # all prediction error in forces
            md_err_n[traj_id] = traj_data[11].tolist()  # all prediction error in NACs
            md_err_s[traj_id] = traj_data[12].tolist()  # all prediction error in SOCs
            md_pop[traj_id] = traj_data[13].tolist()  # all populations
        ...
        t_m = time.time()
        print('Select geometries spent:   ', how_long(t_s, t_m))
        pool.close()
        ...
        t_e = time.time()
        print('Prepare calculation spent: ', how_long(t_m, t_e))
    ...
    def _screen_error_wrapper(self, variables):
        ...
        ## find index of geometries exceeding the dynamical threshold of prediction error
        ...
        if num_uncer_tot == 0:
            ...
            dyn_e_new, itr_e_new = self._dynamical_error(dyn_e, self.dynenergy, self.minenergy, self.maxenergy, itr_e,
                                                         self.fwdenergy, self.bckenergy, ndiscard_e, selec_e)
            dyn_g_new, itr_g_new = self._dynamical_error(dyn_g, self.dyngrad, self.mingrad, self.maxgrad, itr_g,
                                                         self.fwdgrad, self.bckgrad, ndiscard_g, selec_g)
            dyn_n_new, itr_n_new = self._dynamical_error(dyn_n, self.dynnac, self.minnac, self.maxnac, itr_n,
                                                         self.fwdnac, self.bcknac, ndiscard_n, selec_n)
            dyn_s_new, itr_s_new = self._dynamical_error(dyn_s, self.dynsoc, self.minsoc, self.maxsoc, itr_s,
                                                         self.fwdsoc, self.bcksoc, ndiscard_s, selec_s)
    ...
    @staticmethod
    def _sort_errors(err, threshold):
    ...
    @staticmethod
    def _localmax(error, maxsample, neighbor):
    ...
    def _train_model(self):
        ...
        ## start NN training
        if self.itr > 1 or self.load == 0:
            pool = multiprocessing.Pool(processes=1)
            for _ in pool.imap_unordered(self._train_wrapper, [None]):
                _ = None
            pool.close()
...
""" % (
            ...
            np.sum(self.nrefine),
            self.max_e,
            float(np.mean(self.dyn_e)),
            self.max_e <= np.mean(self.dyn_e),
            self.max_g,
            float(np.mean(self.dyn_g)),
            self.max_g <= np.mean(self.dyn_g),
            self.max_n,
            float(np.mean(self.dyn_n)),
            self.max_n <= np.mean(self.dyn_n),
            self.max_s,
            float(np.mean(self.dyn_s)),
        ...
        if len(average_pop) > 0:
            ...
            with open('%s/%s-%s.pop' % (logpath, self.title, self.itr), 'w') as pop:
                pop.write(pop_info)
        ...
        for i in range(self.ntraj):
            ...
            geom_coord += '%s\n%s\n%s\n' % (natom, cmmt, print_coord(np.concatenate((self.atoms, last_geom), axis=1)))
        with open('%s/%s-ntraj-%s-iter-%s.xyz' % (logpath, self.title, self.ntraj, self.itr), 'w') as coord:
            coord.write(geom_coord)

        ## save adaptive sampling results
        # savethis = {self.itr:checkpoint_dict} ## This saves too much !!
